...
async def start_stream(
    ...
    if await is_active_chat(CallbackQuery.message.chat.id):
        ...
        if not await join_stream(CallbackQuery.message.chat.id, file):
            return await mystic.edit("🥺 Kesalahan Bergabung dengan Obrolan Suara.")
...
async def start_stream_audio(
    ...
    if await is_active_chat(message.chat.id):
        ...
        await mystic.delete()
        cap = f"🎥<b>__Playing:__ </b>[Diberikan Audio Melalui Telegram]({link})\n👤**__Diminta oleh:__** {message.from_user.mention}"
        final_output = await message.reply_photo(
            photo="Utils/Telegram.JPEG",
            reply_markup=InlineKeyboardMarkup(buttons),
            caption=cap,
