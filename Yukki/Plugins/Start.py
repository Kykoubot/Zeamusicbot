        ...
        try:
            ...
            if member.id in OWNER_ID:
                return await message.reply_text(
                    f"{MUSIC_BOT_NAME}'s Owner[{member.mention}] baru saja bergabung dengan obrolan Anda."
                )
            if member.id in SUDOERS:
                return await message.reply_text(
                    f"Anggota dari {MUSIC_BOT_NAME}'s Sudo User[{member.mention}] baru saja bergabung dengan obrolan Anda."
                )
            return
        except:
            return
...
async def EVE(_, CallbackQuery):
    ...
    if not is_non_admin:
        ...
        await CallbackQuery.edit_message_text(
            text=f"{text}\n\nAdmins Perintah Mode untuk **Setiap orang**\n\nSekarang siapa pun yang hadir dalam grup ini dapat melewati, jeda, lanjutkan, hentikan musik.\n\nPerubahan Dilakukan Oleh @{checking}",
            reply_markup=InlineKeyboardMarkup(buttons),
        )
    else:
...
async def AMS(_, CallbackQuery):
    ...
    if not is_non_admin:
        ...
        await CallbackQuery.edit_message_text(
            text=f"{text}\n\nSetel Mode Perintah ke **Admins**\n\nSekarang hanya Admin yang ada di grup ini yang dapat melewati, pause, resume, stop musics.\n\nPerubahan Dilakukan Oleh @{checking}",
            reply_markup=InlineKeyboardMarkup(buttons),
        )
...
async def start_markup_check(_, CallbackQuery):
    ...
    if command == "Dashboard":
        ...
        volume = _check["volume"]
        await CallbackQuery.edit_message_text(
            text=f"{text}\n\n**Group:** {c_title}\n**Group ID:** {c_id}\n**Volume Level:** {volume}%\n\nCheck {MUSIC_BOT_NAME}'s Statistik Sistem Di Dasbor Di Sini! Lebih Banyak Fungsi menambahkan segera! Cek terus Support Channel.",
            reply_markup=InlineKeyboardMarkup(buttons),
        )
    if command == "Custommarkup":
        ...
        volume = _check["volume"]
        await CallbackQuery.edit_message_text(
            text=f"{text}\n\n**Group:** {c_title}\n**Group ID:** {c_id}\n**Volume Level:** {volume}%\n**Kualitas Audio:** Default Terbaik",
            reply_markup=InlineKeyboardMarkup(buttons),
        )
    if command == "LV":
        assis = {
    ...
    if command == "DIT":
        ...
        await CallbackQuery.answer(
            f"Zebo Disk Usage: {diske}%", show_alert=True
        )
