...
async def play(_, message: Message):
    ...
    if audio:
        ...
        if not limit:
            return await message.reply_text(
                "**Tidak Ada Batas yang Ditetapkan untuk Panggilan Video**\n\nTetapkan Batas untuk Jumlah Panggilan Video Maksimum yang diizinkan di Bot oleh /set_video_limit [Khusus Pengguna Sudo]"
            )
        ...
        mystic = await message.reply_text(
            "🔄 Sedang Memproses Video... Harap Tunggu!"
        )
        try:
            read = db_mem[message.chat.id]["live_check"]
            if read:
...
async def Music_Stream(_, CallbackQuery):
    ...
    if str(duration) == "None":
        ...
        return await CallbackQuery.edit_message_text(
            "**Streaming Langsung Terdeteksi**\n\Ingin bermain streaming langsung? Ini akan menghentikan musik yang sedang diputar (jika ada) dan akan memulai streaming video langsung.",
            reply_markup=InlineKeyboardMarkup(buttons),
        )
    if CallbackQuery.from_user.id != int(user_id):
        return await CallbackQuery.answer(
            "Ini bukan untukmu! Silakan cari Lagu Anda Sendiri.",
            show_alert=True,
        )
