...
async def admin_risghts(_, CallbackQuery):
    ...
    if command == "pausecb":
        ...
        await CallbackQuery.message.reply_text(
            f"🎧 Obrolan Suara Dijeda by {CallbackQuery.from_user.mention}!",
            ...
            if str(finxx) == "s1s":
                ...
                if int(quality) == 1080:
                    ...
                    final_output = await CallbackQuery.message.reply_photo(
                        ...
                        reply_markup=InlineKeyboardMarkup(buttons),
                        caption=(
                            f"<b>__Dilewati Obrolan Video__</b>\n\n👤**__Diminta oleh:__** {mention}"
                        ),
                    )
                    await mystic.delete()
                else:
...
async def play_playlist(_, CallbackQuery):
    ...
    if not _playlist:
        ...
        await CallbackQuery.message.delete()
        mystic = await CallbackQuery.message.reply_text(
            f"Mulai Daftar Putar Of {third_name}.\n\nDiminta oleh:- {CallbackQuery.from_user.first_name}"
        )
        msg = f"Daftar Putar Antri:\n\n"
        j = 0
        for_t = 0
        for_p = 0
        ...
        if for_p == 1:
            ...
            if await isPreviewUp(preview):
                await CallbackQuery.message.reply_photo(
                    photo=preview,
                    caption=f"Ini adalah Daftar Putar Antri dari {third_name}.\n\nDimainkan by :- {CallbackQuery.from_user.mention}",
                    quote=False,
                    reply_markup=InlineKeyboardMarkup(buttons),
                )
                await m.delete()
...
async def check_playlist(_, CallbackQuery):
    ...
    if not _playlist:
        return await CallbackQuery.answer(
            f"Tidak {genre} Daftar putar di server. Coba tambahkan musik di daftar putar.",
            show_alert=True,
        )
    else:
        j = 0
...
async def del_playlist(_, CallbackQuery):
    ...
    await CallbackQuery.message.reply_text(
        f"Berhasil dihapus {type}'s {genre} seluruh daftar putar\n\nBy :- {CallbackQuery.from_user.mention}"
    )
