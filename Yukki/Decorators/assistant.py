...
async def unban_assistant_(_, CallbackQuery):
    ...
    if not a.can_restrict_members:
        return await CallbackQuery.answer(
            "Saya tidak memiliki izin pengguna untuk melarang/membatalkan larangan. Minta admin mana pun untuk membatalkan pemblokiran asisten.",
            show_alert=True,
        )
    else:
        try:
            ...
            return await CallbackQuery.answer(
                "Gagal membatalkan unban",
                show_alert=True,
            )
        return await CallbackQuery.edit_message_text(
            "Asisten unbanned. Coba Mainkan Sekarang."
        )
        ...
        try:
            ...
            if b.status == "kicked":
                return await message.reply_text(
                    f"Akun Asisten[{ASS_ID}] dicekal.\nBatalkan blokirnya terlebih dahulu untuk menggunakan Bot Musik\n\nUsername: @{ASS_USERNAME}",
                    reply_markup=key,
                )
            if b.status == "banned":
                return await message.reply_text(
